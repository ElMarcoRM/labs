// laba6.6.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
        ...
        if (i != ' ' and i != ',' and i != '.')
            ...
            if (tmp_line != "") {
                ...
                if (words.find(tmp_line) != words.end()) { //find word
                    ...
                    words.insert({ tmp_line, 1 }); //define the word we fond as new object and give it a kye(our word) and element(the number of the word in the text)
                }
    ...
    for (auto i = ++words.begin(); i != words.end(); i++) {
       cout << '\t' << i->first << ": " << i->second / (words["*"] / 100.0) << "%\n";//(iterator) first its the word and secend its our word
    }
}